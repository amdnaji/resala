  ...
  const handleCreate = async () => {
    ...
    try {
      const response = await api.post('/chat/group', {
        title: groupTitle.trim() || t('chat.new_group'),
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
          <h2 className="text-xl font-bold text-gray-800">
            {step === 1 ? t('chat.new_group') : t('chat.group_details')}
          </h2>
        ...
        {step === 1 ? (
          <>
            <div className="p-4 border-b border-gray-50 flex flex-col space-y-4 shrink-0">
              ...
              <div className="text-sm text-gray-500">
                {selectedUserIds.length} {t('chat.participants_selected')} (Min 2)
              </div>
            </div>
                ...
                users.map(u => {
                  ...
                  return (
                    <div
                      ...
                      className="p-3 mb-1 rounded-xl flex items-center space-x-3 rtl:space-x-reverse cursor-pointer hover:bg-gray-100 active:bg-gray-200 transition-colors"
                    >
          ...
          <>
            <div className="p-6 flex flex-col items-center space-y-6 flex-1">
              <div className="w-24 h-24 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 shadow-sm border-4 border-white">
                <Users size={40} />
              </div>
              
              <div className="w-full">
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('chat.group_name')}</label>
                <input
                  autoFocus
                  type="text"
              ...
              <div className="w-full bg-blue-50 text-blue-800 p-4 rounded-lg text-sm">
                <p className="font-medium mb-1">{t('chat.participants')}: {selectedUserIds.length}</p>
                <p className="opacity-80">{t('chat.group_creator_admin_hint') || "You will be the group admin."}</p>
              </div>
            </div>

            <div className="p-4 border-t border-gray-100 flex space-x-3 rtl:space-x-reverse shrink-0">
              <button
                onClick={() => setStep(1)}
              ...
              <button
                ...
                disabled={isLoading}
                className="flex-1 py-2.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading ? <Loader2 className="animate-spin" size={20} /> : t('common.create')}
              </button>
            </div>
          </>
        )}
